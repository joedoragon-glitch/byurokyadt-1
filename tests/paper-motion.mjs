...
try{
 ...
 const corridor=await page.evaluate(()=>window.MinistryAudio.state().corridor);assert.equal(corridor.lastPass.reason,'approach');assert.equal(corridor.lastPass.positions.length,6);assert(corridor.lastPass.positions.every(p=>p[0]>6),'Footsteps originate beyond closed door');
